...
function AppContent() {
  ...
  const userKeyAndIdentityReady =
    userKeyState.isReady && !authIdentity.isPending && !isTamboTokenUpdating;
  ...
  const readyUserKey = userKeyAndIdentityReady
    ? userKeyState.userKey
    : undefined;
  const { data: threadList, isSuccess: threadListLoaded } = useTamboThreadList(
    { userKey: readyUserKey },
    { enabled: userKeyAndIdentityReady },
  );
  ...
  // Reset thread initialization when user key changes (e.g., login/logout)
  React.useEffect(() => {
    if (!userKeyState.isReady) return;

    const currentKey = userKeyState.userKey;
    if (lastUserKeyRef.current !== null && lastUserKeyRef.current !== currentKey) {
...
function TamboAuthedProvider({
  ...
  return (
    <TamboProvider
      ...
      userKey={userKeyState.isReady ? userKeyState.userKey : undefined}
      tools={tools}
